...
#setwd("W:/current_projects/UsT3/Data/Data_analysis/UsT3_lab_data")                                          # The rest of the code is based on 'data' as the starting folder
...
TO <- readRDS("../UsT3_gcms/03_data/geos_mib_als_gcms_data_240816.rds")
#TO <- as.data.frame(TO)
...
write.csv(Summ_tab,"csv_output/ALS/Jul2024_Update/Geosmin_summary_exceedances_Jul24.csv")
...
write.csv(Summ_tab1,"csv_output/ALS/Jul2024_Update/MIB_summary_exceedances_Jul24.csv")
...
## make one table per catchment (ready for the report - this only focuses on the schemes)
 ...
 for (i in catch){
   ...
   print(i)
   write.csv(list_df[[i]], paste0("csv_output/ALS/Jul2024_Update/GEO_summary_exceedances_",i, "_Jul24.csv"))
 }
# 
# 
 for (i in catch){
   ...
   print(i)
   write.csv(list_df[[i]], paste0("csv_output/ALS/Jul2024_Update/MIB_summary_exceedances_",i, "_Jul24.csv"))
 }
...
tiff(paste0("Plots/Taste_odour/Jul2024_Updated/Geosmin_all_sites_boxplots_Jul24.tiff"), width = 200, height = 200, 
...
tiff(paste0("Plots/Taste_odour/Jul2024_Updated/MIB_all_sites_boxplots_Jul24.tiff"), width = 200, height = 200, 
     units = "mm", res = 300, compression = "zip"
     ) # creates a link to save a plot
P2
dev.off()
...
for (i in catch){
  ...
  file_name = paste("Plots/Taste_odour/Jul2024_Updated/GEOSMIN_Boxplots_PerSite_ ", i, "_Jul24.tiff", sep="") # saves the first series of plots
  ...
  p_other <- ggplot(data= subset(TO[which(TO$mib_ng_l>0),], catchment == i), aes(x= code_site, y= mib_ng_l)) +
    ...
    scale_y_continuous(#expand = c(0,0), #limits =c(0,max+1),
                       breaks = seq(0,max(TO$mib_ng_l + 10), 
                                    by=2)
                       )+
    stat_summary(fun.data = stat_box_data,                                      # Use function to add n
  ...
  file_name = paste("Plots/Taste_odour/Jul2024_Updated/MIB_Boxplots_PerSite_ ", i, "_Jul24_2.tiff", sep="") # saves the 2nd series of plots
  tiff(file_name, width = 200, height = 200,  units = "mm", res = 300, compression = "zip"
       )
  print(plot_list[[i]])
  dev.off()
...
for (i in catch) {
  ...
  if (nrow(subset_data) > 0) {
    ...
    file_name <- paste("Plots/Taste_odour/Jul2024_Updated/ALL_TO_Boxplots_PerSite_", i, ".tiff", sep = "") 
...
lims <- as.POSIXct(strptime(c("2022-04-01 00:00","2024-07-15 00:00"), format = "%Y-%m-%d %H:%M"))  
...
for (i in catch){
  file_name = paste("Plots/Taste_odour/Jul2024_Updated/TS_GeosminPerSite_ ", i, "_Jul24.tiff", sep="")
  tiff(file_name, width = 300, height = 200,  units = "mm", res = 300, compression = "zip"
       )
  print(plot_list[[i]])
  dev.off()
...
# Save plots to tiff. Makes a separate file for each plot.
for (i in catch){
  file_name = paste("Plots/Taste_odour/Jul2024_Updated/TS_MIBPerSite_ ", i, "_Jul24.tiff", sep="")
  tiff(file_name, width = 300, height = 200,  units = "mm", res = 300, compression = "zip"
  )
  print(plot_list[[i]])
  dev.off()
...
#Plots/Taste_odour/Mar2023_Updated/MIB_all_sites_boxplots_Mar23.tiff"

# Save plots to tiff. Makes a separate file for each plot.
for (i in catch){
  file_name = paste("Plots/Taste_odour/Jul2024_Updated/BarCharts_GeosminPerSite_Jul24_", i, ".tiff", sep="")
  tiff(file_name, width = 200, height = 200,  units = "mm", res = 300, compression = "zip"
  )
  print(plot_list[[i]])
  dev.off()
...
# Save plots to tiff. Makes a separate file for each plot.
for (i in catch){
  file_name = paste("Plots/Taste_odour/Jul2024_Updated/BarCharts_MIBPerSite_Jul24_", i, ".tiff", sep="")
  tiff(file_name, width = 200, height = 200,  units = "mm", res = 300, compression = "zip"
  )
  print(plot_list[[i]])
  dev.off()
